...
river_ko_intersect2$interpolated_WL_DIFF = predict(model1, data.frame(Distance = river_ko_intersect2$Distance))

river_ko_intersect2 <- river_ko_intersect2 %>% mutate(GWL_near_river = (SmoothWL + interpolated_WL_DIFF)*cof)
...
ko_predWL <- as.data.frame(as_Spatial(ko_river_intersect))
str(ko_predWL)

#river_predWL <- as_tibble(as_Spatial(river_ko_intersect2))
river_predWL <- as.data.frame(as_Spatial(river_ko_intersect2))
str(river_predWL)
...
pred <- ggplot(join_df_clean, aes(Xcoor ,Ycoor, z = GWL_near_river)) +
...
D2River_dataframe = as.data.frame(D2River_Grid2)
