...
```
=======
list_libs <- c("rgdal","readr","smerc","maptools","sf","raster","rasterVis",
               "dismo","spatial","sp","maxstat","RColorBrewer","LaplacesDemon",
               "imager","pracma","rgeos","maps","rnaturalearth","rnaturalearthdata",
               "ggspatial","ggplot2","wesanderson","mltools","gganimate",
               "tools","zoo","lubridate","ggnewscale","gstat","taRifx")
lapply(list_libs, require, character.only = TRUE)
>>>>>>> db1c82309625ae0ec0a9c9bbfb0e848b4869175e
...
#Choose a year to run
WL <- "WL2020Loc.DD.Adjt"
...
str(data)
=======
#filter(WL2019Loc.DD.Adjt != 'NA' & WL != 'NA') 
data <- water_level %>%
  dplyr::select(Longitude, Latitude, SWN, DEM, Depth, ScrTop1, ScrBot1, ScrTop2, 
                ScrBot2, SH_Note, Aq2Use, WL) %>%
                filter(WL != 'NA') 
>>>>>>> db1c82309625ae0ec0a9c9bbfb0e848b4869175e
...
WL_data <- data %>% 
  dplyr::select(Longitude, Latitude, WL) %>%
  filter(WL != 'NA')
>>>>>>> db1c82309625ae0ec0a9c9bbfb0e848b4869175e
...
WL_sp <- SpatialPointsDataFrame(SpatialPoints(WL_data[,1:2]), data.frame(WL_data[,3]))
crs(WL_sp) = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs +towgs84=0,0,0")
...
=======
WL_sp <- spTransform(WL_sp,new_crs)
>>>>>>> db1c82309625ae0ec0a9c9bbfb0e848b4869175e
...
=======
colnames(WL_sp@coords)[1:2] = c("x","y")
colnames(WL_sp@data)        = c(WL)  
>>>>>>> db1c82309625ae0ec0a9c9bbfb0e848b4869175e
...
normalized = function (x) {(x-min(x))/(max(x)-min(x))}  # create a normalization function
#data_sp2019@data$WL2019n <- normalized(data_sp2019@data$WL2019)
WL_sp@data$WL_normalized <- normalized(WL_sp@data[,WL])
>>>>>>> db1c82309625ae0ec0a9c9bbfb0e848b4869175e
...
#v_WL2020n$dist
=======
var_WL_norm <- variogram(WL_normalized ~ 1 , data=WL_sp, cutoff=300000, width=20000)
#str(var_WL_norm)
#summary(var_WL_norm)
#var_WL_norm$np
#var_WL_norm$dist
...
#plot(var_WL_norm, main = "Empirical Variogram",pl = T)

# create a model for the empirical variogram
<<<<<<< HEAD
variogram_model_WL2020n <- vgm(0.05,"Gau",100000,0.003)
=======
variogram_model_WL2020n <- vgm(0.05,"Gau",200000,0.003)
>>>>>>> db1c82309625ae0ec0a9c9bbfb0e848b4869175e
...
#plot(var_WL_norm, pl=T, model=variogram_model_WL2020n, main = "Variogram Model")
#(variogram_model_WL2020n_fit <- fit.variogram(var_WL_norm, variogram_model_WL2020n))
...
# export fig to png
ofile = paste("output/fitted_variogram_",WL,".png")
png(ofile)
plot(var_WL_norm, pl=T, model=variogram_model_WL2020n_fit, main = paste("Fitted OK Variogram Model ", WL))
dev.off()

# Save to a csv file
ofile = paste("output/fitted_variogram_",WL,".csv")
write.csv(var_WL_norm,ofile)
>>>>>>> db1c82309625ae0ec0a9c9bbfb0e848b4869175e
```
